...
public class TierraMedia {
    public static void main(String[] args) {
        ...
        int[] bondadosas = {1, 2, 3, 4, 5};
        int[] malvadas =   {2, 2, 2, 3, 5};

        String[] nombresBondadosas = {"Pelosos", "Sureños buenos", "Enanos", "Númenóreanos", "Elfos"};

        String[] nombresMalvadas = {"Sureños malos", "Orcos", "Goblins", "Huargos", "Trolls"};

        int fuerzaBien = 0;
        int fuerzaMal = 0;

        for (int i = 0; i < bondadosas.length; i++) {
            String input = JOptionPane.showInputDialog("Ingrese la cantidad de " + nombresBondadosas[i] + " (bien):");
            int cantidad = Integer.parseInt(input);
            fuerzaBien += cantidad * bondadosas[i];
        }
        for (int i = 0; i < malvadas.length; i++) {
            String input = JOptionPane.showInputDialog("Ingrese la cantidad de " + nombresMalvadas[i] + " (mal):");
            int cantidad = Integer.parseInt(input);
